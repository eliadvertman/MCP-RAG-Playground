...
class RagMCPServer:
    ...
    Provides MCP tools for document ingestion, semantic search, and collection management
    using the underlying RagAPI implementation.
    """
    
    def __init__(self, rag_api: RagAPI, server_name: str = "RAG Knowledge Base"):
        """
        Initialize the RAG MCP Server.
        
        Args:
            rag_api: Configured RagAPI instance
        ...
        self._setup_prompts()
        logger.info("RAG MCP Server initialization complete")
    
    def _setup_tools(self):
        """Setup MCP tools for RAG operations."""
        
        @self.mcp.tool()
        def add_document_from_file(file_path: str) -> Dict[str, Any]:
            """
            ...
            try:
                ...
                success = self.rag_api.add_document(normalized_path)
                end_time = time.time()
        ...
        @self.mcp.tool()
        def add_document_from_content(content: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
            """
            ...
            try:
                ...
                try:
                    success = self.rag_api.add_document(temp_file_path)
        ...
        def search_knowledge_base(
            ...
            try:
                ...
                results = self.rag_api.query(query.strip(), limit=limit, min_score=min_score)
        ...
        def get_collection_info() -> Dict[str, Any]:
            ...
            try:
                ...
                return {
                    "success": False,
                    "error": str(e),
                    "collection_name": getattr(self.rag_api, 'collection_name', 'unknown'),
                    "status": "error"
                }
        ...
        def delete_collection() -> Dict[str, Any]:
            ...
            """
            try:
                success = self.rag_api.delete_collection()
                collection_name = getattr(self.rag_api, 'collection_name', 'unknown')
